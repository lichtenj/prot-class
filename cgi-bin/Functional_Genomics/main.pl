#!/usr/bin/perl -w
#
# File	  : main.pl -- Main for OU Bioinformatics Bench
# Author  : Ohio University
# Created : July 2005
#
#     Copyright (c) 2005  Ohio University. All rights reserved.
#     This program is free software; you can redistribute it and/or
#     modify it under the same terms as Perl itself.
#
use strict;

#############################################################################
# subclass the application class to separate out specific functionality for
# this specific main app
package main_app;
use application;

use vars qw(@ISA);
@ISA = qw(application);

# override this virtual function to include all the experiment type
# modules you want to "compile" into the exe program
sub Modules
{
  use pmf;
  use hrgp;
  use gps;
  use PromoterPredict;
  
  # return an array of modules used so that
  # the calling function can know which default manifest
  # files to load... for instance
  return (['pmf','hrgp','gps','PromoterPredict']);
}

# override this virtual function to create the new experiment menu specific to
# the modules included in sub Modules()
sub ModulesMenu
{
  my $this = shift or die;
  return
  [
      [qw/command/, 'Protein Mass Fingerprinting (PMF)', -underline=>0, -command => sub{$this->FileNew('pmf');}],
      [qw/command/, 'Hydroxyproline-rich Glycoprotein Analysis (HRGP)', -underline=>0, -command => sub{$this->FileNew('hrgp');}],
      [qw/command/, 'Gravity Persistent Signals Analysis (GPS)', -underline=>0, -command => sub{$this->FileNew('gps');}],
      [qw/command/, 'Promter Prediction (PromoterPredict)', -underline=>0, -command => sub{$this->FileNew('PromoterPredict');}],
  ];
}
############################################################################
package main;

my ($app) = new main_app;
$app->main();

